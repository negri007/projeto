...
| `video/marketing.php` | POST (multipart: `modelo`, `formato`, `nicho?`, `campos` JSON, `foto0..fotoN`) | sim | Gera peça de marketing pelo motor local (render Remotion, $0 de API). Entra na **fila global** do motor (25/09/2026) e devolve `{ok, video_id, status, posicao_fila}`: `status` = `"gerando"` se havia vaga (`posicao_fila: null`) ou `"na_fila"` com a posição (1 = próximo). Uma peça por loja por vez: com outra `na_fila` ou `gerando`, 429 + `{error}`. Provider gravado = `motor` |
| `video/status.php` | GET `?video_id=N` | sim | `{ok, video_id, status, posicao_fila, arquivo, url_plataforma, provider, erro}` (serve tanto o vídeo por IA quanto o do motor). `status`: `na_fila` | `gerando` | `pronto` | `erro`; `posicao_fila` só com `na_fila` (1 = próximo), senão `null` |
| `video/meus.php` | GET | sim | "Meus vídeos" do Comércio (24/09/2026): peças do **motor** (`modelo` preenchido) da loja **da sessão**, 20 mais novas: `{ok, videos:[{id, modelo, formato, status, posicao_fila, arquivo, erro, publicado, created_at}]}`. `status` pode ser `na_fila` (com `posicao_fila`, 1 = próximo; nos outros status vem `null`). `arquivo` só com `status` `pronto`, `erro` só com `erro`; `publicado` = há post ativo da loja com esse vídeo. Sem loja: `{error: "Você ainda não tem loja."}` |
